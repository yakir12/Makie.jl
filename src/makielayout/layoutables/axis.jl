...
function add_cycle_attributes!(allattrs, P, cycle::Cycle, cycler::Cycler, palette::Attributes)
    ...
    if !isempty(manually_cycled_attributes)
        ...
        if !all(k -> k in palettesyms(cycle), manually_cycled_attributes)
            error("Attribute $k was passed with an explicit Cycled, but $k is not specified in the cycler for this plot type $P.")
        end
